...
- [x] `InboxSyncJob` (`modules:progress`) — busca `progreso.json`/`nuevo.json`, valida y aplica entrada por entrada, borra ambos archivos siempre al terminar.
- [x] Verificado localmente con el protocolo nuevo (2026-08-19): creación vía `nuevo.json`, rechazo de id desconocido en `progreso.json`, actualización con `last_modified` distinto, no-op con `last_modified` igual (probado enviando un `progress` distinto con el mismo timestamp — se ignoró correctamente), borrado de ambos archivos tras cada ciclo. Todo verificado por `curl` contra el backend local.
- [x] Migrado de polling (`@Scheduled`) a detección reactiva con `java.nio.file.WatchService` + virtual thread, con debounce de 400ms para coalescer `progreso.json`/`nuevo.json` en un solo ciclo (2026-08-19). Verificado en local con `bootRun`: drop de ambos archivos casi simultáneos procesado en un solo ciclo (~700ms), watch sigue vivo tras el primer ciclo, rechazo por id inexistente sigue funcionando. Ver detalle en `docs/PROGRESS.md`.
